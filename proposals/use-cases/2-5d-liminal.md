...
This use case should be a foundation use case for the existing set. 
It establishes the general treatment of 2D, 2.5D, height-described, jurisdictionally bounded, derived 3D, and authoritative 3D parcel representations. 
It also defines the need to separate geometry `z` values, legal height descriptions, computable vertical limits, height references, vertical extent status, legal-versus-derived status, and provenance. 
Other use cases reuse these concepts when they create solids, define building-referenced boundaries, represent survey-strata or built-strata schemes, model 3D infrastructure parcels, encode mining interests, derive terrain-intersection curves, or interpret water-boundary evidence.

| Related use case                                                                                 | Relationship to this use case                                                                                                                                                                                                                                                                                                                                                            |
|--------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Create a WA 3D CSDM extruded cadastral parcel from a 2D footprint and resolved height definition | This is the direct next-step use case. The height-description use case records the legal wording, vertical extent status, height reference, `zMin`, `zMax`, relative expressions, and computability status. The extruded parcel use case then tests whether those inputs are sufficient to generate a valid closed 3D solid without inventing legal 3D extent.                           |
| Create a WA 3D CSDM Terrain Intersection Curve                                                   | Reuses representation status, vertical extent status, height references, terrain or surface references, legal-versus-derived classification, and provenance. A Terrain Intersection Curve should only be derived where the parcel has a suitable 3D, 2.5D, height-described, or surface-based representation and a stated terrain or ground surface.                                     |
| Encode a WA multi-unit and/or multi-level built-strata scheme as a 3D CSDM dataset               | Reuses the height-description pattern where strata lots, part lots, car bays, storage areas, balconies, courtyards, or common-property-related spaces have height, depth, floor, ceiling, AHD, building-surface, or stratum descriptions. Built-strata geometry should remain legal only where the plan, legislation, title, or approved rule defines it by those building elements.     |
| Represent WA strata wall-boundary definitions between adjoining built-strata lots                | Reuses the distinction between geometry values, height descriptions, legal wording, building surfaces, and derived geometry. Wall, floor, ceiling, centre-plane, joined-building, and inner-surface boundaries may be legal boundary references, but only where the source says so. The wall-boundary use case provides the detailed built-surface boundary pattern.                     |
| Encode a WA multi-unit, multi-level survey-strata scheme as a 3D CSDM dataset                    | Reuses the vertical extent and height-description pattern where survey-strata lots, common property, car bays, storage areas, service areas, easements, or other strata elements are height-limited or depth-limited. Survey-strata boundaries remain survey-defined, and buildings or structures should not be used to imply vertical limits unless the source supports that treatment. |
| Encode WA tunnels, subsurface infrastructure, and airspace parcels as 3D CSDM datasets           | Reuses height descriptions, AHD values, upper and lower limits, natural-surface depth wording, representation status, vertical extent status, and legal-versus-derived classification. The tunnel, subsurface, and airspace use case applies those concepts to plan-defined 3D parcels and interests where the 3D extent is central rather than optional.                                |
| Encode a WA mining tenement survey dataset as a 3D CSDM mining profile                           | Reuses the general 2D, 2.5D, height-described, and 3D representation concepts for vertically limited mining tenements, subsurface rights, exclusion zones, shafts, tunnels, voids, or other mining-related 3D features. The mining profile should add mining-specific classification, vocabulary, marking-out evidence, lifecycle, and affected-land relationships.                      |
| Represent WA natural and general water boundaries as 3D CSDM datasets                            | Reuses the distinction between geometry `z` values, AHD evidence, height or contour references, legal wording, derived geometry, and provenance. A water boundary may use height or AHD evidence, but it is primarily a natural or general boundary pattern, not automatically a vertical parcel extent, derived solid, or terrain-intersection curve.                                   |
