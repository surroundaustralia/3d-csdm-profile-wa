...
It focuses on the specific step of generating, classifying, validating, exchanging, and tracing a closed 3D solid from an authoritative 2D footprint and a resolved height definition. 
Other use cases provide the legal, survey, height, strata, infrastructure, mining, terrain, or natural-boundary contexts that may supply the inputs for extrusion or limit when extrusion is appropriate.

| Related use case                                                                       | Relationship to this use case                                                                                                                                                                                                                                                                                                                                                                                                          |
|----------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Represent a WA 2D parcel with height descriptions and derived 3D extent                | This is the closest prerequisite use case. It defines how to represent 2D, 2.5D, height-described, jurisdictionally bounded, and derived 3D states, including `zMin`, `zMax`, height descriptions, relative height references, AHD values, vertical extent status, and legal-versus-derived status. The extruded parcel use case tests the next step: generating a closed solid only where those height definitions are computable.    |
| Create a WA 3D CSDM Terrain Intersection Curve                                         | May use the resulting extruded solid as input for deriving one or more terrain-intersection curves. A terrain-intersection curve is a derived relationship between parcel geometry and terrain; it should not be confused with the extrusion footprint, the legal vertical limits, or the legal parcel extent unless the source explicitly gives it that status.                                                                       |
| Encode a WA multi-unit and/or multi-level built-strata scheme as a 3D CSDM dataset     | Relevant where built-strata lots, balconies, courtyards, storage lots, car bays, or other strata components have computable vertical limits. However, built-strata boundaries are often defined by building surfaces, walls, floors, ceilings, or plan wording, so extrusion from a 2D footprint should only be used where it accurately reflects the legal boundary definition.                                                       |
| Represent WA strata wall-boundary definitions between adjoining built-strata lots      | Relevant where wall, floor, ceiling, centre-plane, joined-building, or inner-surface boundaries provide computable surfaces or limits for a 3D parcel. The wall-boundary use case should govern the legal boundary basis and physical evidence; the extrusion pattern should not replace detailed wall-boundary modelling unless the source supports a simple extrusion interpretation.                                                |
| Encode a WA multi-unit, multi-level survey-strata scheme as a 3D CSDM dataset          | Relevant where survey-strata lots, car bays, storage areas, service areas, easements, or other strata elements are vertically limited and have computable upper and lower limits. Survey-strata boundaries remain survey-defined, not building-defined, and extrusion should only be applied where the plan supplies a valid footprint and resolved height definition.                                                                 |
| Encode WA tunnels, subsurface infrastructure, and airspace parcels as 3D CSDM datasets | Covers plan-defined 3D parcels and interests that may not be simple vertical extrusions. The extrusion pattern may be used for simple tunnel, subsurface, airspace, or 3D easement cases where a footprint and vertical limits are sufficient, but more complex stepped, sloping, multi-part, non-contiguous, or structure-referenced parcels should follow the dedicated 3D parcel pattern.                                           |
| Encode a WA mining tenement survey dataset as a 3D CSDM mining profile                 | Relevant where a mining tenement, mining-related secondary parcel, exclusion zone, shaft, decline, tunnel, void, or other mining-related feature is legally defined by a footprint and explicit vertical limits. The mining profile should govern mining-specific classification, vocabulary, marking-out evidence, lifecycle, and affected-land relationships; extrusion should not invent a mining volume without legal support.     |
| Represent WA natural and general water boundaries as 3D CSDM datasets                  | Relevant where a parcel with a natural water boundary also has vertical limits that could support a derived solid. The water-boundary geometry, legal basis, evidence, AHD relationship, annotation, and ambulatory behaviour should be established separately before any extrusion is attempted. Natural water boundaries should not be simplified into extruded solids unless the legal and geometric inputs support that treatment. |
