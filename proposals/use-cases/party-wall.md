...
It focuses on wall-related legal boundary definitions between adjoining built-strata lots, including centre-plane, joined-building-plane, inner-surface, floor-surface, ceiling-surface, shared-structure, evidence, database-representation, and encroachment patterns. 
Other use cases provide broader parcel, strata, height, infrastructure, mining, terrain, and natural-boundary contexts that may interact with wall-boundary modelling but should not replace the detailed wall-boundary pattern tested here.

| Related use case                                                                                 | Relationship to this use case                                                                                                                                                                                                                                                                                                                                                                   |
|--------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Represent a WA 2D parcel with height descriptions and derived 3D extent                          | Provides the general treatment of representation status, vertical extent status, `zMin`, `zMax`, height descriptions, AHD references, relative height references, derived surfaces, and legal-versus-derived geometry. This wall-boundary use case reuses those concepts where a wall, floor, ceiling, or building-referenced boundary has vertical limits or height descriptions.              |
| Create a WA 3D CSDM extruded cadastral parcel from a 2D footprint and resolved height definition | Covers generation of a closed 3D solid from a 2D footprint and computable vertical limits. Wall-boundary cases should not be treated as simple extrusions unless the source boundary definition supports that interpretation. The wall-boundary pattern focuses on legal boundary surfaces and physical wall evidence rather than extrusion from a footprint.                                   |
| Create a WA 3D CSDM Terrain Intersection Curve                                                   | Covers derived curves where a 3D or surface-based parcel intersects terrain. Wall, floor, ceiling, slab, and building surfaces should not be treated as terrain or ground surfaces unless the source explicitly defines them that way. A terrain-intersection curve should not replace the legal wall-boundary surface.                                                                         |
| Encode a WA multi-unit and/or multi-level built-strata scheme as a 3D CSDM dataset               | This is the closest companion use case. The built-strata use case provides the broader scheme-level pattern for private lot solids, building evidence, common-property or void treatment, strata element descriptions, and database integration. This wall-boundary use case tests the detailed boundary-definition patterns that can occur within built-strata schemes.                        |
| Encode a WA multi-unit, multi-level survey-strata scheme as a 3D CSDM dataset                    | Covers survey-defined strata lots, common property, easements, optional vertical limits, relevant structures, scheme-level information, and conversion behaviour. Survey-strata boundaries are not building-defined, but wall-boundary evidence may remain relevant as party wall easements, support rights, occupation evidence, conversion lineage, encroachments, or contextual information. |
| Encode WA tunnels, subsurface infrastructure, and airspace parcels as 3D CSDM datasets           | Covers plan-defined 3D parcels and interests such as tunnels, subsurface infrastructure, airspace parcels, and 3D easements. It may reuse the distinction between physical structure and legal boundary where infrastructure, walls, slabs, supports, or encroachments interact with strata lots, but it is focused on 3D infrastructure parcels rather than strata wall-boundary definitions.  |
| Encode a WA mining tenement survey dataset as a 3D CSDM mining profile                           | Generally separate, but relevant where mining-related interests, subsurface infrastructure, support rights, exclusion zones, or affected-land relationships interact with built-strata lots or wall-defined boundaries. Mining tenements should follow the mining-profile pattern; wall-boundary modelling remains limited to strata boundary interpretation and related evidence.              |
| Represent WA natural and general water boundaries as 3D CSDM datasets                            | Generally separate. Natural water boundaries are evidence-based natural or general boundaries, while this use case deals with built-surface boundary definitions. Both use cases require a clear distinction between physical evidence and adopted legal cadastral boundary geometry, but water-boundary evidence and ambulatory behaviour should be handled by the water-boundary use case.    |
