...
It focuses on survey-defined strata lots, common property, easements, optional vertical limits, scheme-level information, strata element descriptions, unit entitlement, relevant structures captured as context, and staged conversion behaviour. 
Other use cases provide related patterns for height handling, derived 3D geometry, building-referenced boundaries, wall-boundary interpretation, 3D infrastructure parcels, mining interests, terrain interaction, and natural boundaries.

| Related use case                                                                                 | Relationship to this use case                                                                                                                                                                                                                                                                                                                                                  |
|--------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Represent a WA 2D parcel with height descriptions and derived 3D extent                          | Provides the general treatment of 2D, 2.5D, height-described, jurisdictionally bounded, and derived 3D representations, including `zMin`, `zMax`, height descriptions, AHD references, undefined extent, unconstrained extent, and legal-versus-derived status. Survey-strata lots may reuse these concepts where vertical limits or height descriptions are supplied.         |
| Create a WA 3D CSDM extruded cadastral parcel from a 2D footprint and resolved height definition | Covers the generation of a closed 3D solid from a 2D footprint and computable vertical limits. This may be relevant for height-limited survey-strata lots, car bays, storage areas, service areas, easements, or other strata elements, but only where the source plan and vertical definition support a computable 3D solid.                                                  |
| Create a WA 3D CSDM Terrain Intersection Curve                                                   | May support cases where a survey-strata lot, common-property area, easement, or vertically limited strata element interacts with terrain. The terrain-intersection curve is a derived relationship and should not replace the surveyed lot boundary, common-property boundary, easement geometry, or legal height description.                                                 |
| Encode a WA multi-unit and/or multi-level built-strata scheme as a 3D CSDM dataset               | Provides the companion pattern for building-referenced strata lots where boundaries are defined by walls, floors, ceilings, slabs, roof surfaces, or other building elements. This survey-strata use case is different because boundaries are survey-defined and should not be derived from buildings or structures.                                                           |
| Represent WA strata wall-boundary definitions between adjoining built-strata lots                | Provides detailed treatment of wall, floor, ceiling, centre-plane, joined-building, inner-surface, shared-structure, and encroachment boundary definitions. This survey-strata use case may reference those patterns for party wall easements, support rights, conversion lineage, or contextual evidence, but should not make survey-strata boundaries building-defined.      |
| Encode WA tunnels, subsurface infrastructure, and airspace parcels as 3D CSDM datasets           | Covers plan-defined 3D parcels and interests such as tunnels, subsurface infrastructure, airspace parcels, and 3D easements. This may be relevant where a survey-strata scheme includes or is affected by 3D easements, service corridors, airspace, support rights, or subsurface infrastructure, but those interests should remain distinct from survey-strata lot geometry. |
| Encode a WA mining tenement survey dataset as a 3D CSDM mining profile                           | Generally separate, but relevant where a mining tenement or mining-related interest affects a survey-strata scheme, common property, easement, service area, or vertically limited strata element. The mining-profile pattern should be used for mining tenement classification and lifecycle, while this use case records the affected survey-strata scheme context.          |
| Represent WA natural and general water boundaries as 3D CSDM datasets                            | Relevant where a survey-strata parcel, common property, accessway, or scheme parcel abuts a water boundary such as HWM, ordinary high water line, low water mark, or watercourse centre thread. The detailed treatment of water-boundary evidence, annotations, AHD relationships, and ambulatory behaviour should follow the water-boundary use case.                         |
